...
class PlayerFollow(arcade.Sprite):

    def follow_sprite(self, mouse_sprite):
        """
        ...
        if self.center_y < mouse_sprite.center_y:
            self.center_y += min(MOVEMENT_SPEED, mouse_sprite.center_y - self.center_y)
        elif self.center_y > mouse_sprite.center_y:
            self.center_y -= min(MOVEMENT_SPEED, self.center_y - mouse_sprite.center_y)

        if self.center_x < mouse_sprite.center_x:
            self.center_x += min(MOVEMENT_SPEED, mouse_sprite.center_x - self.center_x)
        elif self.center_x > mouse_sprite.center_x:
            self.center_x -= min(MOVEMENT_SPEED, self.center_x - mouse_sprite.center_x)
...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.player_sprite = PlayerFollow(":resources:images/space_shooter/playerShip2_orange.png",
    ...
    def on_update(self, delta_time):
        ...
        self.player_sprite.follow_sprite(self.mouse_sprite)
